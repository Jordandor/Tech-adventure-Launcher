...
test('переходы вверх и абсолютные пути запрещены', () => {
  assert.throws(
    () => normalizeManagedPath('../options.txt'),
    /пределы/
  );
  ...
  assert.throws(
    () => normalizeManagedPath('C:\\Windows\\file'),
    /пределы|Недопустимый/
  );
});
...
test('дубликаты и HTTP-ссылки запрещены', () => {
  ...
  duplicated.files.push({
    ...duplicated.files[0],
    path: 'MODS/example.jar'
  });

  assert.throws(
    () => validateManifest(duplicated),
    /повторяется/
  );
  ...
  assert.throws(
    () => validateManifest(outsideGithub),
    /GitHub/
  );
});
...
test('safeDestination остаётся внутри корня', () => {
  ...
  assert.equal(
    safeDestination(root, 'config/test.toml'),
    path.join(root, 'config', 'test.toml')
  );
});
...
test('пакетный манифест поддерживает обновления частями и одноразовые настройки', () => {
  ...
  assert.equal(
    safePackDestination(root, 'options.txt'),
